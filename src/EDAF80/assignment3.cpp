...
edaf80::Assignment3::run()
{
	...
	GLuint const sky_texture = bonobo::loadTextureCubeMap(
	...
	skybox.add_texture("skybox_texture", sky_texture, GL_TEXTURE_CUBE_MAP);
	...
	demo_sphere.set_program(&fallback_shader, set_uniforms);
	demo_sphere.add_texture("skybox_texture", sky_texture, GL_TEXTURE_CUBE_MAP);

	auto my_normal_map_id = bonobo::loadTexture2D(config::resources_path("textures/cobblestone_floor_08_nor_2k.jpg"));
	demo_sphere.add_texture("normal_map", my_normal_map_id, GL_TEXTURE_2D);
	auto my_diffuse_map_id = bonobo::loadTexture2D(config::resources_path("textures/cobblestone_floor_08_diff_2k.jpg"));
